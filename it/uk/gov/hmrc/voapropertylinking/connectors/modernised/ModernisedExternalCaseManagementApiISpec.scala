...
import uk.gov.hmrc.voapropertylinking.stubs.modernised.ModernisedExternalCaseManagementStub
import uk.gov.hmrc.voapropertylinking.utils.HttpStatusCodes.INTERNAL_SERVER_ERROR

import java.time.LocalDateTime
import scala.concurrent.ExecutionContext

class ModernisedExternalCaseManagementApiISpec extends BaseIntegrationSpec with ModernisedExternalCaseManagementStub {
  ...
  trait TestSetup {
    ...
    lazy val connector: ModernisedExternalCaseManagementApi = app.injector.instanceOf[ModernisedExternalCaseManagementApi]
  }
      ...
      "called for client" should {
          ...
          val responseJson: JsValue =
            Json.parse(
              """
                ...
                |}
                |""".stripMargin)
          val expectedModel: CanChallengeResponse = CanChallengeResponse(result = false, reasonCode = Some("any_thing"), reason = Some("anything"))
        ...
        "return None "when {
          "a success status is returned but with invalid json" in new TestSetup {
            ...
            stubCanChallengeClient(propertyLinkSubmissionId, testCheckCaseRef, testValuationId)(OK, Json.obj("incorrect" -> "body"))
          ...
          "any error status is received" in new TestSetup {
            ...
            stubCanChallengeClient(propertyLinkSubmissionId, testCheckCaseRef, testValuationId)(BAD_REQUEST, Json.obj("doesnt" -> "matter"))
      ...
      "called for an agent" should {
        ...
        "return None " when {
          "a success status is returned but with invalid json" in new TestSetup {
            ...
            stubCanChallengeAgent(propertyLinkSubmissionId, testCheckCaseRef, testValuationId)(OK, Json.obj("incorrect" -> "body"))
          ...
          "any error status is received" in new TestSetup {
            ...
            stubCanChallengeAgent(propertyLinkSubmissionId, testCheckCaseRef, testValuationId)(BAD_REQUEST, Json.obj("doesnt" -> "matter"))
