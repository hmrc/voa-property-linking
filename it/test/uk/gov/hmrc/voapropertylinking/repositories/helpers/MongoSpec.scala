...
trait MongoSpec
    extends AnyWordSpec with MongoSupport with GuiceOneAppPerSuite with CleanMongoCollectionSupport with Matchers {
  ...
  implicit class MongoOps(repo: PlayMongoRepository[DVRRecord])(implicit ec: ExecutionContext) {
    ...
    def updateCreatedAtWrongDataType(orgId: Long): UpdateResult =
      await(repo.collection.updateOne(in("organisationId", orgId), set("createdAt", 1)).toFuture())
  }
}
