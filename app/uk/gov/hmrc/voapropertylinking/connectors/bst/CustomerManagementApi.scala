...
class CustomerManagementApi @Inject() (
  ...
  def updateGroupAccount(orgId: Long, account: UpdatedOrganisationAccount)(implicit
    ...
    putRawWithGGHeaders(httpClient, s"$organisationUrl/$orgId", Json.toJsObject(account)).map { _ =>
      ()
    }
  ...
  def getDetailedGroupAccount(
    ...
    getOptionalJsonWithGGHeaders[APIDetailedGroupAccount](httpClient, s"$organisationUrl?organisationId=$id")
  ...
  def withAgentCode(
    ...
    getOptionalJsonWithGGHeaders[APIDetailedGroupAccount](httpClient, s"$organisationUrl?representativeCode=$agentCode")
      .map(_.map(_.toGroupAccount))
  ...
  def createIndividualAccount(account: IndividualAccountSubmission, time: Instant = Instant.now)(implicit
    ...
    postJsonWithGGHeaders[IndividualAccountId](
      httpClient,
      individualUrl,
  ...
  def updateIndividualAccount(personId: Long, account: IndividualAccountSubmission, time: Instant = Instant.now)(
    ...
    putJsonWithGGHeaders[JsValue](
      httpClient,
      individualUrl + s"/$personId",
      Json.toJsObject(account.toAPIIndividualAccount(time))
    )
  ...
  def getDetailedIndividual(
    ...
    getOptionalJsonWithGGHeaders[APIDetailedIndividualAccount](httpClient, s"$individualUrl?personId=$id")
