...
class EnvelopeInfoTest extends BaseUnitSpec {

  "EnvelopeInfo" should {
    "be deserialised property" in {
      val envelope = Json.parse(DummyData.envelopeContent).as[EnvelopeInfo]
      envelope.status shouldBe "OPEN"
      envelope.files.map(_.id) shouldBe Seq("index.jpeg", "index2.jpeg")
    }
  }

  "EnvelopeInfo.files" should {
    "be optional" in {
      val envelope = Json.parse(DummyData.envelopeWithNoFiles).as[EnvelopeInfo]
      envelope.status shouldBe "OPEN"
      envelope.files shouldBe Nil
    }
  }
}
