...
class AssessmentServiceSpec extends BaseUnitSpec {
  ...
  "getMyOrganisationsAssessments" should {
    "return assessments" in new Setup {
      when(mockExternalPropertyLinkApi.getMyOrganisationsPropertyLink(mEq(submissionId))(any(), any()))
        .thenReturn(Future.successful(Some(ownerPropertyLink)))
      when(mockExternalValuationManagementApi.getValuationHistory(mEq(uarn), mEq(submissionId))(any(), any()))
  ...
  "getClientsAssessments" should {
    "return assessments" in new Setup {
      when(mockExternalPropertyLinkApi.getClientsPropertyLink(mEq(submissionId))(any(), any()))
